...
def build_dll_for_arch(args, dll_path, cpp_paths, cu_paths, arch, libs: list[str] | None = None, mode=None):
    ...
    if os.name == "nt":
        ...
        cpp_flags = f'-Werror -Wuninitialized {version} --std=c++17 -fno-rtti -D{cuda_enabled} -D{hip_define} -D{ck_define} -D{mathdx_enabled} -D{cuda_compat_enabled} -fPIC -fvisibility=hidden -fvisibility-inlines-hidden -D_GLIBCXX_USE_CXX11_ABI={cxx11_abi} -I"{native_dir}" {includes} '
        ...
        with ScopedTimer("link", active=args.verbose):
            ...
            unexpected = [sym for sym in undefined if not sym.startswith(("Py", "_Py", "_ZN4llvm", "_ZNK4llvm", "ZSTD_", "LLVM"))]
            if unexpected:
